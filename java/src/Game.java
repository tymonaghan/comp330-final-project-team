...
class Game {
    ...
    void play(Host myHost, Scanner scanner, Player playerOne, Player playerTwo) {
        ...
        QuestionFiles qf = new QuestionFiles("/" + myHost.getDifficulty());
        QuestionFiles af = new QuestionFiles("/answers.txt");
        ...
        if (myHost.getLevel() == 1 || myHost.getLevel()==2) { // if difficulty is EASY or MEDIUM, show 3x multiple choice options
            QuestionFiles choices = new QuestionFiles("/choices.txt"); // read in the multiple choices for each question
            int userResponse=0;
            ...
            QuestionFiles answerMatches = new QuestionFiles("/hardstrings.txt");
